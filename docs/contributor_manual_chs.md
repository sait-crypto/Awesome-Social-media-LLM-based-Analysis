...
- `assets/figures/`：存放引用图片（论文截图、pipeline 图等），更新图片需在 PR 中一并提交。
- `assets/papers/`：存放论文 PDF 文件（可选，用于 AI 分析）。
...
GUI 新增功能：
- **文件管理**：可直接在 `Pipeline图` 和 `论文文件` 字段点击 📂 浏览选择文件，或点击 👁️ 打开文件。`Pipeline图` 支持点击 📋 粘贴剪贴板图片。系统会自动将文件复制到 `assets/` 目录下。
- **AI 辅助**：
    - 点击 `⚙️ AI设置` 配置 DeepSeek 或 Gemini 模型。
    - 点击 `✨ 生成所有空字段` 让 AI 读取 PDF 并自动填写摘要、总结、翻译等。
    - 点击 `🏷️ AI分类建议` 获取分类指导。
- **分类树**：点击 `🌳 查看分类树` 浏览当前的分类层级结构。
